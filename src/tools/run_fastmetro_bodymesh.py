...
from src.utils.comm import all_gather, get_rank, is_main_process
...
from src.utils.renderer_opendr import (
    ...
    visualize_reconstruction_smpl_opendr,
)
    ...
    def forward(self, pred_vertices, gt_vertices, has_smpl, device):
        ...
        if len(coord_gt) > 0:
            if self.uniform:
                d1_out = torch.sqrt(
                    torch.sum(
                        (coord_out[:, edge[:, 0], :] - coord_out[:, edge[:, 1], :]) ** 2,
                        2,
                        keepdim=True,
                    )
                    + 1e-8
                )  # batch_size X num_edges X 1
                d1_gt = torch.sqrt(
                    torch.sum(
                        (coord_gt[:, edge[:, 0], :] - coord_gt[:, edge[:, 1], :]) ** 2,
                        2,
                        keepdim=True,
                    )
                    + 1e-8
...
def run_train(
    ...
    mesh_sampler,
    renderer,
    smpl_intermediate_faces,
    smpl_intermediate_edges,
):
    smpl.eval()
    ...
    edge_gt_loss = EdgeLengthGTLoss(
        smpl_intermediate_faces, smpl_intermediate_edges, uniform=args.uniform
    )
    edge_self_loss = EdgeLengthSelfLoss(
        smpl_intermediate_faces, smpl_intermediate_edges, uniform=args.uniform
    )
    ...
    if args.resume_epoch > 0:
        log_eval_metrics_mpjpe.set(
            epoch=args.resume_mpjpe_best_epoch,
            mPVPE=args.resume_mpjpe_best_mpvpe,
            mPJPE=args.resume_mpjpe_best_mpjpe,
            PAmPJPE=args.resume_mpjpe_best_pampjpe,
        )
        log_eval_metrics_pampjpe.set(
            epoch=args.resume_pampjpe_best_epoch,
            mPVPE=args.resume_pampjpe_best_mpvpe,
            mPJPE=args.resume_pampjpe_best_mpjpe,
            PAmPJPE=args.resume_pampjpe_best_pampjpe,
        )
    ...
    for _, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        gt_2d_joints = annotations["joints_2d"].cuda(
            args.device
        )  # batch_size X 24 X 3 (last for visibility)
        ...
        # gt 3d joints
        gt_3d_joints = annotations["joints_3d"].cuda(
            args.device
        )  # batch_size X 24 X 4 (last for confidence)
        ...
        gt_3d_joints[:, :, :3] = (
            gt_3d_joints[:, :, :3] - gt_3d_pelvis[:, None, :]
        )  # batch_size X 14 X 4
        ...
        gt_3d_vertices_intermediate = mesh_sampler.downsample(
            gt_3d_vertices_fine, n1=0, n2=1
        )  # batch_size X 1723 X 3
        gt_3d_vertices_coarse = mesh_sampler.downsample(
            gt_3d_vertices_intermediate, n1=1, n2=2
        ...
        gt_3d_vertices_fine = (
            gt_3d_vertices_fine - gt_smpl_3d_pelvis[:, None, :]
        )  # batch_size X 6890 X 3
        gt_3d_vertices_intermediate = (
            gt_3d_vertices_intermediate - gt_smpl_3d_pelvis[:, None, :]
        )  # batch_size X 1723 X 3
        gt_3d_vertices_coarse = (
            gt_3d_vertices_coarse - gt_smpl_3d_pelvis[:, None, :]
        )  # batch_size X 431 X 3

        # forward-pass
        out = FastMETRO_model(images)
        pred_cam, pred_3d_joints_from_token = out["pred_cam"], out["pred_3d_joints"]
        pred_3d_vertices_coarse, pred_3d_vertices_intermediate, pred_3d_vertices_fine = (
            out["pred_3d_vertices_coarse"],
            out["pred_3d_vertices_intermediate"],
            out["pred_3d_vertices_fine"],
        )

        # obtain 3d joints, which are regressed from the full mesh
        pred_3d_joints_from_smpl = smpl.get_h36m_joints(
            pred_3d_vertices_fine
        )  # batch_size X 17 X 3
        pred_3d_joints_from_smpl_pelvis = pred_3d_joints_from_smpl[
            :, cfg.H36M_J17_NAME.index("Pelvis"), :
        ]
        ...
        pred_3d_vertices_fine = (
            pred_3d_vertices_fine - pred_3d_joints_from_smpl_pelvis[:, None, :]
        )  # batch_size X 6890 X 3
        pred_3d_vertices_intermediate = (
            pred_3d_vertices_intermediate - pred_3d_joints_from_smpl_pelvis[:, None, :]
        )  # batch_size X 1723 X 3
        pred_3d_vertices_coarse = (
            pred_3d_vertices_coarse - pred_3d_joints_from_smpl_pelvis[:, None, :]
        )  # batch_size X 431 X 3
        # normalize predicted joints
        pred_3d_joints_from_smpl = (
            pred_3d_joints_from_smpl - pred_3d_joints_from_smpl_pelvis[:, None, :]
        )  # batch_size X 14 X 3
        pred_3d_joints_from_token_pelvis = (
            pred_3d_joints_from_token[:, 2, :] + pred_3d_joints_from_token[:, 3, :]
        ) / 2
        pred_3d_joints_from_token = (
            pred_3d_joints_from_token - pred_3d_joints_from_token_pelvis[:, None, :]
        )  # batch_size X 14 X 3
        # obtain 2d joints, which are projected from 3d joints of smpl mesh
        pred_2d_joints_from_smpl = orthographic_projection(
            pred_3d_joints_from_smpl, pred_cam
        )  # batch_size X 14 X 2
        pred_2d_joints_from_token = orthographic_projection(
            pred_3d_joints_from_token, pred_cam
        )  # batch_size X 14 X 2
        ...
        loss_3d_vertices = (
            args.vertices_coarse_loss_weight
            * vertices_loss(
            ...
            * vertices_loss(
            ...
            + args.vertices_fine_loss_weight
            * vertices_loss(
                criterion_3d_vertices,
                pred_3d_vertices_fine,
                gt_3d_vertices_fine,
        ...
        loss_edge_normal = (
            ...
            * edge_gt_loss(
            ...
            + args.edge_self_loss_weight
            * edge_self_loss(pred_3d_vertices_intermediate, has_smpl, args.device)
            + args.normal_loss_weight
            * normal_loss(
                pred_3d_vertices_intermediate,
                gt_3d_vertices_intermediate,
                has_smpl,
        ...
        loss_2d_joints = keypoint_2d_loss(
            ...
            criterion_2d_keypoints, pred_2d_joints_from_smpl, gt_2d_joints, has_2d_joints
        )
        ...
        if args.use_smpl_param_regressor:
            ...
            pred_smpl_3d_joints_pelvis = pred_smpl_3d_joints[
                :, cfg.H36M_J17_NAME.index("Pelvis"), :
            ]
            pred_smpl_3d_joints = pred_smpl_3d_joints[
                :, cfg.H36M_J17_TO_J14, :
            ]  # batch_size X 14 X 3
            pred_smpl_3d_vertices = (
                pred_smpl_3d_vertices - pred_smpl_3d_joints_pelvis[:, None, :]
            )  # batch_size X 6890 X 3
            pred_smpl_3d_joints = (
                pred_smpl_3d_joints - pred_smpl_3d_joints_pelvis[:, None, :]
            )  # batch_size X 14 X 3
            pred_smpl_2d_joints = orthographic_projection(
                pred_smpl_3d_joints, pred_cam.clone().detach()
            )  # batch_size X 14 X 2
            loss_smpl_3d_joints = keypoint_3d_loss(
                criterion_3d_keypoints,
                pred_smpl_3d_joints,
                gt_3d_joints,
                has_3d_joints,
            ...
            loss_smpl_2d_joints = keypoint_2d_loss(
                criterion_2d_keypoints, pred_smpl_2d_joints, gt_2d_joints, has_2d_joints
            )
            loss_smpl_vertices = vertices_loss(
                criterion_3d_vertices,
                pred_smpl_3d_vertices,
                gt_3d_vertices_fine,
                has_smpl,
        ...
        if (
            (epoch != 0)
            and ((epoch % args.saving_epochs) == 0)
        ...
        if (iteration % iters_per_epoch) == 0:
            ...
            val_result = run_validate(
                args, val_dataloader, FastMETRO_model, epoch, smpl, mesh_sampler
            )
            val_mPVPE, val_mPJPE, val_PAmPJPE = (
                val_result["val_mPVPE"],
                val_result["val_mPJPE"],
                val_result["val_PAmPJPE"],
            )
            if args.use_smpl_param_regressor:
                val_smpl_mPVPE, val_smpl_mPJPE, val_smpl_PAmPJPE = (
                    val_result["val_smpl_mPVPE"],
                    val_result["val_smpl_mPJPE"],
            ...
            if is_main_process():
                if args.use_smpl_param_regressor:
                    logger.info(
                        ...
                        + "  MPVPE: {:6.2f}, MPJPE: {:6.2f}, PA-MPJPE: {:6.2f} ".format(
                            1000 * val_mPVPE, 1000 * val_mPJPE, 1000 * val_PAmPJPE
                        )
                ...
                logger.info(
                    ...
                    + " <MPVPE> {:6.2f} <MPJPE> {:6.2f}, <PA-MPJPE> {:6.2f}, at Epoch {:6.2f}".format(
                        1000 * log_eval_metrics_pampjpe.mPVPE,
                        1000 * log_eval_metrics_pampjpe.mPJPE,
                        1000 * log_eval_metrics_pampjpe.PAmPJPE,
                        log_eval_metrics_pampjpe.epoch,
                    )
                )
                logger.info(
                    "Best Results (MPJPE):"
                    + " <MPVPE> {:6.2f}, <MPJPE> {:6.2f}, <PA-MPJPE> {:6.2f}, at Epoch {:6.2f}".format(
                        1000 * log_eval_metrics_mpjpe.mPVPE,
                        1000 * log_eval_metrics_mpjpe.mPJPE,
                        1000 * log_eval_metrics_mpjpe.PAmPJPE,
                        log_eval_metrics_mpjpe.epoch,
                    )
                )
...
def run_eval(args, val_dataloader, FastMETRO_model, smpl, renderer):
    ...
    val_result = run_validate(args, val_dataloader, FastMETRO_model, epoch, smpl, renderer)
    val_mPVPE, val_mPJPE, val_PAmPJPE = (
        val_result["val_mPVPE"],
        val_result["val_mPJPE"],
        val_result["val_PAmPJPE"],
    ...
    with torch.no_grad():
        for iteration, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            if args.use_smpl_param_regressor:
                pred_rotmat, pred_betas = out["pred_rotmat"], out["pred_betas"]
                pred_smpl_3d_vertices = smpl(pred_rotmat, pred_betas)  # batch_size X 6890 X 3
                pred_smpl_3d_joints = smpl.get_h36m_joints(
                    pred_smpl_3d_vertices
                )  # batch_size X 17 X 3
                pred_smpl_3d_joints_pelvis = pred_smpl_3d_joints[
                    :, cfg.H36M_J17_NAME.index("Pelvis"), :
                ...
                # measure errors
                error_smpl_vertices = mean_per_vertex_position_error(
                    pred_smpl_3d_vertices, gt_3d_vertices_fine, has_smpl
                )
                error_smpl_joints = mean_per_joint_position_error(
                    pred_smpl_3d_joints, gt_3d_joints, has_3d_joints
                )
                error_smpl_joints_pa = reconstruction_error(
                    pred_smpl_3d_joints.cpu().numpy(),
                ...
                if len(error_smpl_joints_pa) > 0:
                    smpl_PAmPJPE.update(
                        np.mean(error_smpl_joints_pa), int(torch.sum(has_3d_joints))
                    )

            # measure errors
            error_vertices = mean_per_vertex_position_error(
                pred_3d_vertices_fine, gt_3d_vertices_fine, has_smpl
            )
            ...
            if args.run_eval_and_visualize:
                ...
                if args.use_smpl_param_regressor:
                    pred_smpl_2d_joints = orthographic_projection(
                        pred_smpl_3d_joints, pred_cam
                    )  # batch_size X 14 X 2
                    visual_imgs = visualize_mesh_with_smpl(
                        renderer,
                        annotations["ori_img"].detach(),
                        pred_3d_vertices_fine.detach(),
                        pred_cam.detach(),
...
def visualize_mesh_with_smpl(renderer, images, pred_vertices, pred_cam, pred_smpl_vertices):
    ...
    for i in range(batch_size):
        ...
        # Visualize reconstruction
        if args.use_opendr_renderer:
            rend_img = visualize_reconstruction_smpl_opendr(
                img, vertices, cam, renderer, smpl_vertices
            )
        else:
            rend_img = visualize_reconstruction_smpl_pyrender(
                img, vertices, cam, renderer, smpl_vertices
            )
        rend_img = rend_img.transpose(2, 0, 1)
        rend_imgs.append(torch.from_numpy(rend_img))
...
def parse_args():
    ...
    parser.add_argument(
        "--train_yaml",
        default="Tax-H36m-coco40k-Muco-UP-Mpii/train.yaml",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--val_yaml",
        default="human3.6m/valid.protocol2.yaml",
        type=str,
        required=False,
...
def main(args):
    ...
    smpl_intermediate_faces = torch.from_numpy(
        np.load(
            "./src/modeling/data/smpl_1723_faces.npy",
            encoding="latin1",
            allow_pickle=True,
        ).astype(np.int64)
    ).to(args.device)
    smpl_intermediate_edges = torch.from_numpy(
        np.load(
            "./src/modeling/data/smpl_1723_edges.npy",
            encoding="latin1",
            allow_pickle=True,
        ).astype(np.int64)
    ).to(args.device)
