...
def visualize_reconstruction_opendr(
    img, vertices, camera, renderer, color="light_blue", focal_length=1000
):
    """
...
def visualize_reconstruction_multi_view_opendr(
    img, vertices, camera, renderer, color="light_blue", focal_length=1000
):
    # Fix a flength so i can render this with persp correct scale
    res = img.shape[1]
...
def visualize_reconstruction_smpl_opendr(
    img, vertices, camera, renderer, smpl_vertices, color="light_blue", focal_length=1000
):
    # Fix a flength so i can render this with persp correct scale
    res = img.shape[1]
    camera_t = np.array([camera[1], camera[2], 2 * focal_length / (res * camera[0] + 1e-9)])
